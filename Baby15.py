...
class TextFileManager:
    def __init__(self, data_dir: str = "multi_agent_data", conversation_limit: int = 70):
        ...
        self.data_dir = Path.home() / "LAZYCOOK_data"
        self.file_lock = Lock()
        ...
        try:
            ...
            if not os.access(file_path, os.R_OK):
                ...
                try:
                    ...
                    logger.error(f"Cannot change permissions: {perm_error}")
                    return None
            ...
            if file_type.startswith('text/') or file_type in ['application/json', 'application/xml']:
                ...
                for encoding in encodings:
                    try:
                        ...
                        logger.warning(f"Failed to read with {encoding}: {enc_error}")
                        continue

                if not content_read or not content.strip():
            ...
            if not content:
                ...
                content = f"[Empty file: {file_path.name}]"
            ...
            try:
                ...
                logger.error(f"Failed to calculate hash: {hash_error}")
    ...
    def debug_document_context(self, user_id: str):
        """Debug method to verify documents are readable and in context"""
        docs = self.get_user_documents(user_id, 10)

        logger.info(f"=== DOCUMENT DEBUG for {user_id} ===")
        ...
        for i, doc in enumerate(docs, 1):
            ...
            logger.info(f"  Readable: {doc.metadata.get('readable', 'unknown')}")

        # Check if documents appear in context
        context = self.get_conversation_context(user_id, 5)
        logger.info(f"\nContext includes documents: {'RELEVANT DOCUMENTS' in context}")
    ...
    @log_errors
    def verify_document_readable(self, document_id: str, user_id: str) -> Dict[str, Any]:
        """Verify a document can be read and return diagnostic info"""
        documents = self.get_user_documents(user_id, 1000)
    ...
    async def _optimize_solution(self, user_query: str, context: str, previous_iteration: Dict) -> AgentResponse:
        ...
        try:
            ...
            if not response.candidates or not response.candidates[0].content.parts:
                logger.warning(f"Optimizer blocked by safety filter")
                return AgentResponse(
                    agent_role=self.role,
                    content=original_solution,
                    confidence=0.70,
                    suggestions=["Content flagged by safety filter"],
                    errors_found=[],
                    improvements=[],
            ...
            try:
                ...
                logger.warning(f"Failed to parse JSON from optimizer: {json_err}")
                return AgentResponse(
                    agent_role=self.role,
                    content=original_solution,
                    confidence=0.75,
                    suggestions=["JSON parsing failed - using original"],
                    errors_found=[],
                    improvements=[],
                    metadata={"json_parse_error": str(json_err)}
                )
    ...
    def get_agent_pipeline(self, complexity: str) -> dict:
        ...
        pipelines = {
            ...
            'medium': {
                ...
                'use_optimizer': True,
                'use_validator': False,
                'max_iterations': 2,  # Change 1 to 2
                'description': 'Balanced response (3-6 API calls)'  # Update description
            },
            'complex': {
                'use_generator': True,
                'use_analyzer': True,
                'use_optimizer': True,
                'use_validator': True,
                'max_iterations': 3,  # Changed from 2 to 3
                'description': 'High-quality response (4-12 API calls)'
            }
        }
...
class MultiAgentSystem:
    ...
    async def process_query(self, user_query: str, context: str = "",
        ...
        # Iteration-based quality thresholds (more realistic)
        iteration_thresholds = {
            0: 0.92,  # First attempt target (realistic)
            1: 0.90,  # Second attempt target
            2: 0.88,  # Third attempt acceptable
            3: 0.86  # Final attempt minimum
        }
        ...
        while current_iteration < pipeline['max_iterations']:
            ...
            if can_exit:
                ...
                # Check for safety block or JSON error
                opt_metadata = iteration_data.get('optimizer_response', {}).get('metadata', {})
                if opt_metadata.get('safety_blocked') or opt_metadata.get('json_parse_error'):
                    logger.warning("⚠ Optimizer issue detected - will retry")

                # Don't exit early - let the loop continue
                current_iteration += 1

                # Only break if we've truly hit max iterations
                if current_iteration >= pipeline['max_iterations']:
                    logger.info(f"→ Reached max iterations ({pipeline['max_iterations']})")
                    break

                # Check for safety block in optimizer
                opt_metadata = iteration_data.get('optimizer_response', {}).get('metadata', {})
                if opt_metadata.get('safety_blocked'):
                    logger.warning("⚠ Safety block detected - will retry in next iteration")

                # Check if we've hit max iterations
                if current_iteration + 1 >= pipeline['max_iterations']:
                    logger.info(f"→ Reached max iterations ({pipeline['max_iterations']}), accepting current quality")
                    break
...
class RichMultiAgentCLI:
    ...
    def display_response(self, user_message: str, ai_response: str, processing_time: float, context_used: str = ""):
        ...
        user_panel = Panel(
            Text(user_message, style="italic blue"),  # Remove the "👤 You:" prefix
            title="👤 Your Message",  # Keep only the title with emoji
            border_style="blue",
            padding=(1, 2),
    ...
    def upload_document(self):
        ...
        if not file_path.exists():
            ...
            self.console.print(f"[yellow]Current directory: {os.getcwd()}[/yellow]")
            return None
    ...
    def test_document_reading(self):
        """Test if documents are actually being read"""
        docs = self.assistant.file_manager.get_user_documents(self.user_id)
        ...
        for doc in docs:
            info = self.assistant.file_manager.verify_document_readable(doc.id, self.user_id)
            self.console.print(f"\n[bold]Document: {doc.filename}[/bold]")
            self.console.print(f"Has content: {info.get('has_content', False)}")
            self.console.print(f"Content length: {info.get('content_length', 0)}")
            self.console.print(f"Preview: {info.get('content_preview', 'N/A')[:100]}")
    ...
    async def run_interactive(self):
        ...
        try:
            while True:
                ...
                elif command == "test-docs":
                    self.assistant.file_manager.debug_document_context(self.user_id)
                    Prompt.ask("[bold]Check logs for detailed debug info. Press Enter to continue...[/bold]")
...
def create_assistant(api_key: str, conversation_limit: int = 70):
    ...
    config = MultiAgentAssistantConfig(api_key, conversation_limit)
